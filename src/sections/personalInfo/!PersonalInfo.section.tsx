...
export default function PersonalInfo() {
	...
	return (
		<section className="p-relative">
			...
			<div className="row">
				<div className="col-4">
					<div className="uppercase fw-medium fs-sm">Mateusz Eiding</div>
					<div className="fs-xs d-flex flex-sm-column flex-print-row column-gap-3">
						<a href={email && `mailto:${email}`}>{email ?? "email"}</a>
						<div className="v-divider d-sm-none d-print-block" />
						<a
							href={
								phone && `tel:${phone.replaceAll(" ", "").replaceAll("-", "")}`
							}
						>
							{phone ?? "phone"}
						</a>
